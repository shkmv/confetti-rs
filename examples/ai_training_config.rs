...
"#;
    ...
    if let Some(model) = conf_unit
        .directives
        .iter()
        .find(|d| d.name.value == "model")
    {
        // Get the model type
        ...
        if let Some(arch) = model
            .children
            .iter()
            .find(|d| d.name.value == "architecture")
        {
                    ...
                    if layer.name.value == "layer" {
                        ...
                        match layer_type.as_str() {
                            ...
                            "dense" => {
                                ...
                                // Remove quotes for display
                                let activation =
                                    if activation.starts_with('"') && activation.ends_with('"') {
