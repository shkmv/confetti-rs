...
#[cfg(feature = "derive")]
use confetti_rs::{
    ConfDirective, ConfArgument,
    FromConf, ToConf, MapperError,
    from_str, to_string, ConfMap
};
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    impl FromConf for ServiceConfig {
        fn from_directive(directive: &ConfDirective) -> Result<Self, MapperError> {
            ...
            if directive.name.value != "ServiceConfig" {
                return Err(MapperError::ParseError(
                    format!("Expected directive name ServiceConfig, found {}", directive.name.value)
                ));
            }
            ...
            let version = directive.children.iter()
            ...
            let database_child = directive.children.iter()
            ...
            let server_child = directive.children.iter()
    ...
    println!("    Max Pool Size: {:?}", service_config.database.max_pool_size);
    println!("  Server:");
