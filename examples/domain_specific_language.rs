...
"#;
                ...
                match directive.name.value.as_str() {
                    ...
                    "add" => {
                        if self.stack.len() >= 2 {
                            ...
                            println!("  Add {} + {} = {} -> Stack: {:?}", a, b, result, self.stack);
                        }
                    },
                    "pop" => {
                        if !self.stack.is_empty() && !directive.arguments.is_empty() {
                            let value = self.stack.pop().unwrap();
                            let var_name = directive.arguments[0].value.trim_start_matches('$').to_string();
                    ...
                    "func" => {
                        if !directive.arguments.is_empty() {
                            ...
                            println!("  Define function: {} with parameters: {:?}", func_name, params);
    ...
    fn print_control_flow(directives: &[confetti_rs::ConfDirective], indent: usize) {
        ...
        for directive in directives {
            match directive.name.value.as_str() {
                ...
                "if" => {
                    if !directive.arguments.is_empty() {
                        println!("{}If condition: {}", indent_str, directive.arguments[0].value);
                        print_control_flow(&directive.children, indent + 1);
                    }
                },
                "else" => {
                ...
                "exit" => {
                    if !directive.arguments.is_empty() {
                        println!("{}Exit with code: {}", indent_str, directive.arguments[0].value);
                    }
                },
                _ => {
                ...
                "states" => {
                    ...
                    for state in &directive.children {
                        ...
                        for action in &state.children {
                            let args = action.arguments.iter()
                                .map(|a| a.value.clone())
                                .collect::<Vec<String>>()
                                .join(" ");
    ...
    if let Some(states) = state_conf.directives.iter().find(|d| d.name.value == "states") {
        for state in &states.children {
            ...
            println!("");
            println!("        self.states['{}'] = state_{}", state.name.value, state.name.value);
            println!("");
        }
    }
    
    // Generate code for events
    if let Some(events) = state_conf.directives.iter().find(|d| d.name.value == "events") {
        for event in &events.children {
            ...
            for action in &event.children {
                if action.name.value == "goto_state" && !action.arguments.is_empty() {
                    println!("            self.current_state = '{}'", action.arguments[0].value);
                    println!("            self.states[self.current_state](entity)");
                }
            }
            
            println!("");
            println!("        self.events['{}'] = event_{}", event.name.value, event.name.value);
            println!("");
        }
    }
