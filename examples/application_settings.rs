...
"#;
    ...
    if let Some(app_section) = conf_unit.directives.iter().find(|d| d.name.value == "application") {
        if let Some(version) = app_section.children.iter().find(|d| d.name.value == "version") {
    ...
    if let Some(display_section) = conf_unit.directives.iter().find(|d| d.name.value == "display") {
        if let Some(resolution) = display_section.children.iter().find(|d| d.name.value == "resolution") {
